...
class RatingDonutView @JvmOverloads constructor(context: Context, attributeSet: AttributeSet?=null) : View(context, attributeSet) {
    ...
    private fun initPaint() {
        ...
        strokePaint = Paint().apply {
            ...
            //кладем значение из поля класса, потому как у нас краски будут видоизменяться
            strokeWidth = stroke
            //Цвет будем получать в специальном методе, потому что в зависимости от рейтинга
            //будем менять цвет нашего кольца
    ...
    private fun drawRating(canvas: Canvas) {
        ...
        //Сохраняем канвас
        canvas.save()
        //Перемещаем нулевые координаты канваса в центр, так проще рисовать круглое
    ...
    private fun drawText(canvas: Canvas) {
        //Форматируем текст, чтобы дробное число с одной цифрой после точки
