...
import nacc.sergey.watchmovie.viewmodel.DetailsFragmentViewModel

class DetailsFragment : Fragment() {

    private lateinit var film: Film
    private lateinit var binding: FragmentDetailsBinding
    //скоуп для Корутины с загрузкой постера
    ...
    private fun performAsyncLoadOfPoster() {
        ...
        //Создаем родительский скоуп с диспатчером Main потока,
        //будет взаимодействовать с UI
        MainScope().launch {
    ...
    //Узнаем, было ли получено разрешение у пользователя ранее
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            val contentValues = ContentValues().apply {
                ...
                put(
                    ...
                    film.title.handleSingleQuote()          //handleSingleQuote кастомный метод для того, чтобы убирать кавычки
                )
    ...
    //handleSingleQuote — это кастомный метод для того, чтобы убирать кавычки.
    //они в названии фильма или описании влияют на составление пути не лучшим образом
