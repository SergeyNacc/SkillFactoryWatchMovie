...
class FilmListRecyclerAdapter(private val clickListener: OnItemClickListener) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    fun addItems(list: List<Film>) {
        items.clear() //Сначала очищаем(если не реализовать DiffUtils)
        items.addAll(list) //Добавляем
        notifyDataSetChanged()  //Уведомляем RV, что пришел новый список, и ему нужно заново все "привязывать"
    }
